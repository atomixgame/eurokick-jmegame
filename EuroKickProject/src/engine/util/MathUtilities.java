...
public final class MathUtilities {
    ...
    /**
     * from :
     * http://stackoverflow.com/questions/2422712/c-rounding-integer-division-up-instead-of-truncating
     */
    ...
    /**
     * Returns the rotation between 2 positions based on X axis. Range [-PI,
     * PI]. Left Of : 0 = -0 Top Of : HALF_PI Right Of : PI = -PI Bottom Of :
     * -HALF_PI
     */
    ...
    public static Vector3f findIntersectionPoint(Vector3f p0, Vector3f p1, Vector3f planeOrigin, Vector3f planeNormal) {
        ...
        Vector3f r0 = p0.interpolateLocal(p1, t);
    ...
    /**
     * Symmetry formula from Koen Samyn :
     * http://knol.google.com/k/mirroring-a-point-on-a-3d-plane# Same info on
     * wikipedia : http://en.wikipedia.org/wiki/Reflection_%28mathematics%29
     *
     * @param pointToBeMirrored the Point you want to find its symmetric
     * @param planeOrigin the planeOrigin e.g (0,0,0)
     * @param planeNormal the planeNormal is a vector that points upward from
     * the plane e.g Vector3f.UNIT_Z
     */
    ...
    public Vector3f getPerpendicular(Vector3f p1) {
        ...
        if (axisDistanceX <= axisDistanceY && axisDistanceX <= axisDistanceZ) {
            p2 = Vector3f.UNIT_X;
        } else if (axisDistanceY <= axisDistanceX && axisDistanceY <= axisDistanceZ) {
            p2 = Vector3f.UNIT_Y;
        }
    ...
    /**
     * returns a random float from [min, max). Max is exclusive i.e it never
     * takes that value.
     */
