...
import sg.games.football.geom.C2DMatrix;
...
public class MovingEntity extends BaseGameEntity {
    ...
    public void update(float tpf) {
        ...
        if (this.getSpatial() != null) {
            //FIXME: convert the position in 2D to 3d space

            // Translation: acording to the Pitch center
            Vector3f real3DPos = FootballEntityManager.getInstance().getDefaultTranslation(position);
            ...
            float angle = (float) Math.acos(heading.x);
            float HALF_PI = 1.5708f;
            Quaternion quat = new Quaternion();

            quat.fromAngleAxis(angle + HALF_PI, Vector3f.UNIT_Y);
            getSpatial().setLocalRotation(quat);
            //println "BaseGameEntity update" + ID

            /*
             Transform t = EntityManager.getInstance().getDefaultTranslation(position,heading);
             getSpatial().setLocalTransform(t);
             */
        }
    ...
    boolean rotateHeadingToFacePosition(Vector2D target) {
        Vector2D toTarget = Vec2DNormalize(target.minus(position));
