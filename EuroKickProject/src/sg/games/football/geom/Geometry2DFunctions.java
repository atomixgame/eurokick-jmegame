...
public class Geometry2DFunctions {
    ...
    public static SpanType whereIsPoint(Vector2D point,
            ...
            Vector2D PlaneNormal) {
        Vector2D dir = PointOnPlane.minus(point);
    ...
    public static double GetRayCircleIntersect(Vector2D RayOrigin,
        ...
        Vector2D ToCircle = CircleOrigin.minus(RayOrigin);
...
//        Vector2D Point = A + ((B - A) * dotA) / (dotA + dotB);
        Vector2D Point = A.plus(B.minus(A).multiply(dotA).divide(dotA + dotB));
...
//            point = A + r * (B - A);
            point = A.plus(B.minus(A).multiply(r));
            return true;
        } else {
            dist = 0;
    ...
    boolean PointInCircle(Vector2D Pos,
            double radius,
            Vector2D p) {
        double DistFromCenterSquared = p.minus(Pos).LengthSq();
    ...
    boolean GetLineSegmentCircleClosestIntersectionPoint(Vector2D A,
        ...
        Vector2D toBNorm = Vec2DNormalize(B.minus(A));
        ...
        if ((LocalPos.x + radius >= 0)
            ...
            if (Math.abs(LocalPos.y) < radius) {
                ...
                IntersectionPoint = A.plus(toBNorm).multiply(ip);
            }
        }
