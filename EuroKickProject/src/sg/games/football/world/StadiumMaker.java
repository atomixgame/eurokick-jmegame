 ...
 */
package sg.games.football.world;
...
public class StadiumMaker {
    ...
    public StadiumMaker(Node rootNode, AssetManager assetManager) {
        ...
        worldTestHelper = new WorldTestHelper(rootNode, assetManager);
    }
    ...
    private void makeSprite4(int currentIndex, Vector3f pos) {
    ...
    private void makeLane(Vector3f pos1, Vector3f pos2, int numOfLayer, int currentIndex, boolean make4) {
        Vector3f pos = new Vector3f();
        for (int j = 0; j < numOfLayer; j++) {

            pos.interpolateLocal(pos1, pos2, 1f / numOfLayer * j);
            if (make4) {
    ...
    public void configStadium() {
        ...
        triggerNode.getChild("conner1").removeFromParent();
        triggerNode.getChild("conner2").removeFromParent();
        triggerNode.getChild("conner3").removeFromParent();
        triggerNode.getChild("center").removeFromParent();
    }
    ...
    public void attachStadium(PhysicsSpace physicsSpace, AppStateManager stateManager) {
        ...
        worldTestHelper.gizmo.setLocalTranslation(center);
    ...
    private void createPLByWayPoint() {
    }

    private void createPLByFalling(int n) {
    ...
    private void createByPointList(int incStep, boolean make4) {
        ...
        while (currentIndex < pointList1.size()) {
            ...
            if (startLinePoint.distance(pointList1.get(currentIndex)) > minDis) {
                ...
                for (int i = 0; i < incStep; i++) {
                    ...
                    pos1.interpolateLocal(startLinePoint, endLinePoint, value);
                    pos2.interpolateLocal(pointList2.get(startIndex), pointList2.get(currentIndex), value);
    ...
    public Vector3f getGoalKeeperPos(int i) {
        return getConnerLoc(1).clone().interpolateLocal(getConnerLoc(2), 0.5f);
    }
